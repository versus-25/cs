...
* **Физические джойны**
* **Nested Loops Join** (далее будем называть его соединением вложенных циклов) сравнивает каждую строку одной таблицы (называемой внешней таблицей) с каждой строке другой таблицы (называемой внутренней таблицей), ища те строки, которые удовлетворяют предикату соединения.
про конкурентность
...
```
#### Типы yеизменяемые (в функциях передается независимая копия)
